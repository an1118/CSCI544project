...
from xor_train import TrainModel

# sys.path.append('/Users/jack/repo/CDIL-CNN/experiments')
sys.path.append('/content/drive/My Drive/CDIL-CNN/experiments')
print(sys.path)
...
os.makedirs('xor_log', exist_ok=True)
os.makedirs('xor_model', exist_ok=True)
log_file_name = './xor_log/' + file_name + '.txt'
model_name = './xor_model/' + file_name
...
# Data
data_train, labels_train = torch.load(f'/content/drive/My Drive/CDIL-CNN/xor_datasets/{TASK}_{SEQ_LEN}_train.pt')
...
data_val, labels_val = torch.load(f'/content/drive/My Drive/CDIL-CNN/xor_datasets/{TASK}_{SEQ_LEN}_val.pt')
val_set = DatasetCreator(data_val, labels_val)
val_loader = DataLoader(val_set, batch_size=BATCH, shuffle=False, drop_last=False)

data_test, labels_test = torch.load(f'/content/drive/My Drive/CDIL-CNN/xor_datasets/{TASK}_{SEQ_LEN}_test.pt')
